...
public class Asteroid extends Participant implements ShipDestroyer
{
    ...
    /**
     * When an Asteroid collides with an AsteroidDestroyer, and BulletDestroyer, it expires.
     */
    @Override
    public void collidedWith (Participant p)
    {
        /**
         * When the Asteroid collides with the AsteroidDestroyer, it expires the asteroid and adds the debris
         */
        if (p instanceof AsteroidDestroyer)
        {
            ...
            // Inform the controller that the asteroid has been destroyed and adds the debris
            controller.addParticipant(new Debris(0, p.getX(), p.getY()));
            controller.addParticipant(new Debris(0, p.getX() + 1, p.getY() + 1));
            controller.addParticipant(new Debris(0, p.getX() + 0.5, p.getY() + 0.5));
            controller.addParticipant(new Debris(0, p.getX() - 1, p.getY() - 1));
            controller.asteroidDestroyed();

        }
        /**
         * When the Asteroid collides with the Ship, it expires the ship
         */
