...
import javax.swing.*;
import asteroids.participants.newAsteroid;
import asteroids.participants.newShip;
import asteroids.participants.newAlienship;
import asteroids.participants.Bullets;
import asteroids.participants.New_Bullet;
import asteroids.participants.newDebris;
import sounds.Sound;

/**
 * Controls a game of newAsteroids.
 */
public class Enhanced_controller implements KeyListener, ActionListener, Iterable<Participant> {
	/** The state of all the Participants */
	private ParticipantState pstate;
	/** The ship (if one is active) or null (otherwise) */
	private newShip ship;
	/** The alienShip (if one is active) or null (otherwise) */
	private newAlienship alienShip;
	...
	/**Records the highscore for the current session*/
	private int highscore;
	
	public int recordtotasteroids;
	
	public int recordtotaliens;

	/**
	 * Constructs a Enhanced_controller to coordinate the game and screen
	 */
	public Enhanced_controller() {
		...
		//set highscore to zero when application is opened
		highscore = 0;
		
		//set total aliens destroyed to zero
		recordtotaliens = 0;
		
		//set total asteroids destroyed to zero
		recordtotasteroids = 0;
	}

	/**
	 * This makes it possible to use an enhanced for loop to iterate through the
	 * Participants being managed by a Enhanced_controller.
	 */
	@Override
	...
	private void splashScreen() {
		...
		clear();
		display.setLegend("Enhanced Asteroids", "");
	...
	private void finalScreen() {
		...
		display.setLegend(GAME_OVER , " Final Score is " + score );
	...
	private void placeShip() {
		...
		ship = new newShip(SIZE / 2, SIZE / 2, -Math.PI / 2, this);
		addParticipant(ship);
	...
	/**
	 * Finds a randomNumber to be set as the speed of the asteroid, which should be
	 * non negative and not be zero
	 */
	...
	/**
	 * Finds a randomVariety to be set as the speed of the asteroid, which should be
	 * a random number
	 */
	...
	/**
	 * Places an asteroid near one corner of the screen. Gives it a random velocity
	 * and rotation.
	 */
	private void placenewAsteroids() {
		// Top left asteroid
		addParticipant(new newAsteroid(findRandomVariety(3), 2, EDGE_OFFSET - 20, EDGE_OFFSET - 20,
				findRandomNumber(MAXIMUM_LARGE_ASTEROID_SPEED), this));
		// Top Right newAsteroid
		addParticipant(new newAsteroid(findRandomVariety(3), 2, EDGE_OFFSET + 550, EDGE_OFFSET + 50,
				(MAXIMUM_LARGE_ASTEROID_SPEED), this));
		// Bottom Left newAsteroid
		addParticipant(new newAsteroid(findRandomVariety(3), 2, EDGE_OFFSET - 100, EDGE_OFFSET + 550,
				findRandomNumber(MAXIMUM_LARGE_ASTEROID_SPEED), this));
		// Bottom Right newAsteroid
		addParticipant(new newAsteroid(findRandomVariety(3), 2, EDGE_OFFSET + 550, EDGE_OFFSET + 550,
				findRandomNumber(MAXIMUM_LARGE_ASTEROID_SPEED), this));
	...
	/**
	 * A helper method to place new asteroids so that every time the level
	 * increments, the number of asteroids go up by one
	 */
	private void placeNewnewAsteroid() {
		if (lives > 0 && ship != null && level > 1) {
			for (int i = 1; i < level; i++) {
				addParticipant(new newAsteroid(findRandomVariety(3), 2, EDGE_OFFSET + findRandomVariety(700),
	...
	public void placeAlienShip() {
		...
		if (level == 2 && alienShip == null) {
			alienShip = new newAlienship(RANDOM.nextInt(750), RANDOM.nextInt(750), this, 1);
		...
		if (level >= 3 && alienShip == null) {
			...
			alienShip = new newAlienship(RANDOM.nextInt(750), RANDOM.nextInt(750), this, 0.5);
			addParticipant(alienShip);
		}
	}
	...
	/**
	 * A helper method to alternate the beat, everytime a new game is clicked or a
	...
	public void shipDestroyed() {
		...
		// Since the ship was destroyed, schedule a transition and sets all the booleans
		// to false
	...
	public void alienShipDestoyed() {
		...
		alienShip = null;
		if (alienShip == null) {
			// Finds the random transition time for the alienShip showing up the next time
			// should be between 5 and 10
			// seconds
			int newTransition = 1000;
	...
	private void teleport() {
		Participant.expire(ship);
		ship = new newShip(RANDOM.nextInt(750), RANDOM.nextInt(750), RANDOM.nextInt(360), this);
		addParticipant(ship);
	}
	/**
	...
	public void splitAsteroid(int variety, double x, double y, int size, int speed) {
		addParticipant(new newAsteroid(findRandomVariety(variety), size, x+30, y+30, findRandomNumber(speed), this));
		addParticipant(new newAsteroid(findRandomVariety(variety), size, x-30, y-30, findRandomNumber(speed), this));
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Time to refresh the screen and deal with keyboard input
			...
			{
				display.switchLifeLight(true);
			}
			else
			{
				display.switchLifeLight(false);
			}
			...
			try {
				...
				if (bullet) {
					Bullets = new New_Bullet(ship.getXNose() - 3, ship.getYNose(), ship, this);
	...
	private void performTransition() {
		...
		if (transitionTime <= System.currentTimeMillis()) {
			...
			// Increments the level, adds new asteroids, places a new ship, and alternates
			// the beat if meets all the
			// conditions
	...
	/**
	 * If a key of interest is pressed, record that it is down and performs a
	 * movement based on the key pressed.
	 */
	@Override
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_L && ship != null) {
			right = true;
		}
		if (e.getKeyCode() == KeyEvent.VK_J && ship != null) {
			left = true;
		}
		if (e.getKeyCode() == KeyEvent.VK_I && ship != null) {
			accelerate = true;
			ship.setMoving(true);
		}
		if (e.getKeyCode() == KeyEvent.VK_K && ship != null) {
			bullet = true;
			sound.fire();
		}
